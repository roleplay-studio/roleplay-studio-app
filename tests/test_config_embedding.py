...
def test_settings_from_env_uses_openrouter_default_for_embedding_base_url(monkeypatch):
    """Unset EMBEDDING_BASE_URL → falls back to OPENROUTER_BASE_URL.
...
def test_settings_from_env_embedding_api_key_unset_stays_none(monkeypatch):
    ...
    ``openrouter_api_key``. This split is what lets the
    ``test_http_embeddings_does_not_fall_back_to_openrouter_api_key``
...
def test_settings_from_env_embedding_api_key_empty_is_no_auth(monkeypatch):
    """EMBEDDING_API_KEY=*** → no auth (None), even if OPENROUTER_API_KEY is set."""
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-test")
...
def test_settings_from_env_embedding_api_key_explicit_value(monkeypatch):
    """EMBEDDING_API_KEY='***' → use that value, ignore openrouter."""
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-test")
...
def test_settings_from_env_embedding_api_key_unset_no_openrouter(monkeypatch):
