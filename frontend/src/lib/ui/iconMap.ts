// Each icon is a complete <svg> element string for use with {@html}
...
export function renderIcon(name: string, size: number = 16): string {
  ...
  if (svg) {
    // Replace width/height with desired size if needed
    return svg
      .replace(/width="\d+"/, `width="${size}"`)
      .replace(/height="\d+"/, `height="${size}"`);
  }
  // Fallback: return empty span (icon missing)
  console.warn(`[iconMap] Missing icon: "${name}"`);
  return `<span class="icon-missing" title="Missing icon: ${name}"></span>`;
}
