...
const SENTINEL_DOC = `e2e-rag-${Date.now()}: The Cult of Vaermina watches from the night; Vaermina governs dreams and omens in Tamriel's oldest libraries.`;
const FILE_NAME = `e2e-rag-${Date.now()}.txt`;

backendTest('upload knowledge → query through real Chroma returns the chunk @smoke', async ({
  backend,
}) => {
  ...
  const up = await postMultipartText(
    backend.api,
    `/api/knowledge/${botId}/upload`,
    FILE,
    PROBE,
  );
...
backendTest('knowledge list returns uploaded entries via DTO projection @smoke', async ({
  backend,
}) => {
  await backend.waitReady();
  const list = await backend.api.get('/api/bots');
  ...
  expect(ours.length, 'at least one chunk from our upload must appear in the listing').toBeGreaterThan(0);
