...
backendTest('state/regenerate: bad assistant_message_id returns 404 @smoke', async ({
  backend,
}) => {
  ...
  expect(r.status(), `bad assistant_message_id should reject; got body: ${await r.text()}`).toBe(404);

  await backend.api.delete(`/api/threads/${threadId}`);
});

backendTest('chat send → messages endpoint returns user+assistant rows even when LLM errors @smoke', async ({
  backend,
}) => {
  await backend.waitReady();
  ...
  const msgs = (await (await backend.api.get(`/api/threads/${threadId}/messages?limit=20`)).json()) as Array<{
    id: number;
    role: string;
    content: string;
    state: string | null;
  ...
  expect(assistantRow, 'assistant row missing — DB write is LLM-conditional, that is wrong').toBeTruthy();
  ...
  expect('state' in assistantRow!, 'MessageDTO.state not projected — frontend will lose world-state UI').toBeTruthy();
